...
@section('title', 'iPorsa || Blog Detail page')
...
<div class="comment-list" data-id="{{ $comment->id }}">
    <!-- Display Comment Content -->
    <div class="single-comment">
        <!-- Check if Comment User Info is not null -->
        @if ($comment->user_info)
            <img src="{{ $comment->user_info['avatar'] }}" alt="{{ $comment->user_info['name'] }}">
            <div class="content">
                <h4>{{ $comment->user_info['name'] }}</h4>
                <!-- Display Comment Content -->
                <p>{{ $comment->comment }}</p>
            </div>
        @endif
        <!-- Check if Comment Replies exist -->
        @if ($comment->replies)
            <!-- Display Replies -->
            @include('frontend.pages.comment', ['comments' => $comment->replies, 'post_id' => $post_id, 'depth' => $depth - 1])
        @endif
        <!-- Check if Authenticated User -->
        @auth
            <!-- Reply Button -->
            <a href="#" class="btn-reply reply" data-id="{{ $comment->id }}">Reply</a>
        @else
            <!-- Display a message if not authenticated -->
            <p>You need to <a href="{{ route('login.form') }}" style="color: rgb(54, 54, 204)">Login</a> OR <a style="color: blue" href="{{ route('register.form') }}">Register</a> for a comment.</p>
        @endauth
    </div>
</div>
@endforeach
